...
export const PALETTE_VISION: OrbPalette = {
  base:   0xffd700,
  think:  0xffec8b,
  speak:  0xffa500,
  bright: 0xfffacd,
...
export const PALETTE_ADJOUA: OrbPalette = {
  base:   0xff7f00,
  think:  0xffa54f,
  speak:  0xff4500,
  bright: 0xffd39b,
...
export function createOrb(canvas: HTMLCanvasElement, palette: OrbPalette = PALETTE_VISION): Orb {
  let destroyed = false;
  const N = 2500;
  ...
  // ── Colour helpers ─────────────────────────────────────────────────────────
  const COL_BASE   = new THREE.Color(palette.base);
  const COL_THINK  = new THREE.Color(palette.think);
  const COL_SPEAK  = new THREE.Color(palette.speak);
  const COL_BRIGHT = new THREE.Color(palette.bright);
  ...
  function animate() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // ── Octahedron containment ──
      const distManhattan = (Math.abs(x) + Math.abs(y) + Math.abs(z)) || 0.01;
      const dist = Math.sqrt(x * x + y * y + z * z) || 0.01;
      const radiusTarget = (speaking || demoActive)
        ? currentRadius * (1.0 + Math.sin(t * 3.5 + px * 0.2) * 0.15 * breathAmp)
        : currentRadius;

      // Force de rappel vers la surface du losange (octaèdre)
      const pullBase = demoCollapse
        ? Math.max(0, distManhattan - radiusTarget) * 0.015 + 0.002
        : Math.max(0, distManhattan - radiusTarget) * 0.002 + 0.0003;
      
      vel[i3] -= (x / distManhattan) * pullBase;
      vel[i3 + 1] -= (y / distManhattan) * pullBase;
      vel[i3 + 2] -= (z / distManhattan) * pullBase;

      // ── Bass push (proportionnel à la forme) ──
      if (bass > 0.05) {
        const bf = (speaking || demoActive) ? bass * 0.038 : bass * 0.025;
        vel[i3] += (x / distManhattan) * bf;
        vel[i3 + 1] += (y / distManhattan) * bf;
        vel[i3 + 2] += (z / distManhattan) * bf;
      }
    ...
    } else if (speaking || externalVolume > 0.05) {
      const spkVol = Math.max(bass, externalVolume);
      mat.opacity = Math.min(1.0, 0.85 + spkVol * 0.4 + shockwave * 0.3);
      mat.size = 0.55 + spkVol * 0.45 + shockwave * 0.4;

      const pulseIntensity = (spkVol * 0.8 + mid * 0.3 + shockwave * 0.5);
      const wave = 0.5 + 0.5 * Math.sin(t * 15.0 + spkVol * 10.0);
      _tmpColor.lerpColors(COL_SPEAK, COL_BRIGHT, Math.min(1, pulseIntensity * wave));
      _tmpColor.lerp(COL_FLASH, 0.45 + spkVol * 0.5); // Effet surbrillant intense

      mat.color.lerp(_tmpColor, 0.25);
      lineMat.color.lerp(_tmpColor, 0.25);
      lineMat.opacity = lineAmount * 0.25 + spkVol * 0.3;
